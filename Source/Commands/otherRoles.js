...
module.exports = {
    ...
    let aether = args[0];
    if (!aether || !member) return message.channel.send(olumsuz.setDescription(`Komutu doğru kullanmalısın. \`Örnek: ${ayar.botPrefix || ""}rol vip @üye\``).setFooter(`.rol bilgi | kullanarak bilgi alabilirsiniz.`)).then(x => x.delete({timeout: 12000}));

    if (aether === "bilgi" || aether === "info") {
        message.channel.send(embed.setDescription(`
        ${ayar.botPrefix || ""}rol vip @üye \`>\` Belirtilen üyeye **VIP** rolü verir/alır.
        ${ayar.botPrefix || ""}rol muzisyen @üye \`>\` Belirtilen üyeye **Müzisyen** rolü verir/alır.
        ${ayar.botPrefix || ""}rol vokal @üye \`>\` Belirtilen üyeye **Vokal** rolü verir/alır.
        ${ayar.botPrefix || ""}rol terapist @üye \`>\` Belirtilen üyeye **Terapist** rolü verir/alır.
        ${ayar.botPrefix || ""}rol soruncözücü @üye \`>\` Belirtilen üyeye **Sorun Çözücü** rolü verir/alır.
        ${ayar.botPrefix || ""}rol lovers @üye \`>\` Belirtilen üyeye **Lovers** rolü verir/alır.
        ${ayar.botPrefix || ""}rol streamer @üye \`>\` Belirtilen üyeye **streamer** rolü verir/alır.
         `))
        return;
    }
    ...
    if (aether === "streamer") {
        let streamerrol = message.guild.roles.cache.get(ayar.streamerrol);
        if (!member.manageable) return message.channel.send(olumsuz.setDescription(`Bu üye üzerinde işlem gerçekleştiremiyorum.`)).then(x => x.delete({timeout: 10000}));
         if (streamerrol && !member.roles.cache.has(ayar.streamerrol)) {
             member.roles.add(streamerrol)
             message.channel.send(olumlu.setDescription(`${member} adlı üyeye başarılı bir şekilde ${streamerrol} rolü verdim.`))
             message.react("✅");
         } 
        if (streamerrol && member.roles.cache.has(ayar.streamerrol)) {
            member.roles.remove(streamerrol)
            message.channel.send(olumlu.setDescription(`${member} adlı üyeden başarılı bir şekilde ${streamerrol} rolünü aldım.`))
            message.react("✅")
         } 
         if (!streamerrol) {
             message.channel.send(olumsuz.setDescription(`Lovers rolü bulunamadı.`))
